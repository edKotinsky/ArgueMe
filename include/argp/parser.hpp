...
namespace argp {
  ...
  public:
     ...
     * Parses a vector of arguments.
     *
     * In a `while` loop goes through the vector, and tries to find each string
     * in an `args` dictionary. If the current string is found, i.e it is a
     * named argument, then calls its `parse` method.
     *
     * Otherwise, if current string is not found, checks if there are positional
     * arguments. If so, assigns current string to a positional argument and
     * increments a counter of positional arguments.
     *
     ...
     * argument. If an end of input vector is reached, then assigns to an input
     * vector's pointer a null value. In the case if input vector is null,
  ...
  private:
    ...
    std::unordered_map<std::string_view, std::reference_wrapper<argument>> args;
  ...
  public:
    command_line(std::string_view longname_start,
                 std::string_view shortname_start);
    void attach(argument& arg);
    void parse(char const* argv, int argc);
    void parse(command_line_impl&);
  private:
    command_line_impl impl;
  };

  template <typename T>
  class value_argument : public argument,
                         public argument_template<T> {
  public:
    ...
    virtual void parse(command_line_impl&) override final {}
  ...
  template <typename T>
  class multi_argument : public argument,
                         public argument_template<T> {
  public:
  ...
  class switch_argument : public argument,
                          public argument_template<bool> {
  public:
